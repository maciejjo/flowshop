...
int Instance::RandomScheduling(Machine machines[3]) {
  ...
  printf("(Instance::RandomScheduling) ======================== \n");
    ...
    for(int i = 0; i < 3; i++) {
      if((machines[i].state == 0) && (machines[i].job_queue.size() > 0 )) {
        ...
        printf("(Instance::RandomScheduling)[%d] | Zadanie przechodzi na %d maszynę:\t%d[%d|",
          time, i, machines[i].job_queue.back()->id, machines[i].job_queue.back()->ready_time);
        for(int j = 0; j < 3; j++) printf(" %d", machines[i].job_queue.back()->operation_times[j]);
        printf("]\n");

        machines[i].state = 1;
        machines[i].job = *machines[i].job_queue.back();
        machines[i].job_queue.pop();
    ...
    for(int i = 0; i < 3; i++) {
      if((machines[i].state) && (machines[i].job.operation_times[i] == 0)) {
        printf("(Instance::RandomScheduling)[%d] == Moduł przekładający zadania na kolejki\n", time);
        printf("(Instance::RandomScheduling)[%d] ----------\n", time);

        printf("(Instance::RandomScheduling)[%d] | Zadanie zostało wykonane na maszynie %d:\t%d[%d|",
        ...
        machines[i].state = 0;
        if(i < 2) {
          machines[i+1].job_queue.push(&machines[i].job);
        }
    ...
    for(int i = 0; i < 3; i++) {
      if((machines[i].state == 1) && (machines[i].job.operation_times[i] > 0)) {
        ...
        printf("(Instance::RandomScheduling)[%d] | Zadanie wykonuje się na maszynie %d[%d]:\t%d[%d|",
          time, i, machines[i].state, machines[i].job.id, machines[i].job.ready_time);
        for(int j = 0; j < 3; j++) printf(" %d", machines[i].job.operation_times[j]);
        printf("]\n");
...
//    if(jobs.empty() && ready_jobs.empty() && machines[0].state == 0 && machines[1].state == 0 && machines[2].state == 0) break;
    ...
    if(jobs.empty() && ready_jobs.empty() && machines[0].state == 0 && machines[1].state == 0 && machines[2].state == 0 && stop) break;
